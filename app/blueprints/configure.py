...
def configure():
    if request.method == 'POST':
        ...
        if section == 'voting':
            ...
            try:
                association_name = request.form.get('association_name', '').strip()
                _save('ASSOCIATION_NAME', association_name)
                configure_logger.info(f"Association name updated: {association_name}")
