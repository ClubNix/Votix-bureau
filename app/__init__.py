...
def create_app():
    ...
    app.config['ASSOCIATION_NAME'] = os.getenv('ASSOCIATION_NAME', 'Votix')
        ...
        try:
            ...
            with _sqlite3.connect(_db_path) as _conn:
                ...
                if 'logo' not in _cols:
                    ...
                    _conn.commit()
                if 'role_id' not in _cols:
                    _conn.execute('ALTER TABLE candidates ADD COLUMN role_id INTEGER REFERENCES roles(id)')
                    _conn.commit()
    ...
    def inject_globals():
        ...
        _cfg = _dv(os.path.join(os.path.dirname(__file__), '.env'))
        return {
            'is_armed': os.path.isfile(pubkey_path),
            'association_name': _cfg.get('ASSOCIATION_NAME') or app.config.get('ASSOCIATION_NAME', 'Votix'),
        }
